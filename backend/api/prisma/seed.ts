...
import {
  ENTITY_ALIASES_BY_SLUG,
  ENTITY_EN_BY_SLUG,
  RELATION_EN_BY_KEY,
  RELATION_TYPES,
  SOURCE_EN_BY_KEY,
  SOURCE_REF_EN_BY_KEY,
} from './seed.fixtures';
import type { ExplicitEntityTranslation } from './seed.fixtures';
...
async function seedRelationAndSourceRefTranslations() {
  ...
  for (const relation of relations) {
    const relationKey = `${relation.from.slug}::${relation.relationType.key}::${relation.to.slug}`;
