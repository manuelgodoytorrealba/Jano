...
import { ForceLayoutScratch, applyAmbientGraphDrift } from './graph-layout';
...
export function advanceExplorerLoop(options: {
  ...
  if (options.camera.viewportTarget) {
    options.camera.animateViewport(
  ...
  if (imageAnimation) shouldContinue = true;
